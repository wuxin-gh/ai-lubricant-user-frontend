...
export enum ConstsNotifyChannelKind {
  ...
  /** 站内通知中心（铃铛 + App 推送）；无 webhook_url，由服务端写 notifications 行。 */
  NotifyChannelNotifyCenter = "notify_center",
}
...
export interface DomainAudit {
  ...
  id?: string;
  operation?: string;
  /** 操作对象摘要（后端从 request 提取的邮箱/名称/id），供表格「具体值」列直接展示 */
  target?: string;
  request?: string;
  response?: string;
  source_ip?: string;
...
export interface DomainCreateNotifyChannelReq {
  ...
  kind: "dingtalk" | "feishu" | "wecom" | "webhook" | "notify_center";
  /** @maxLength 64 */
  name: string;
  secret?: string;
  webhook_url: string;
...
/** 技术栈识别 profile（stack_detector 输出；null = 未扫/扫描中/失败） */
export interface DomainStackProfile {
  /** 依赖清单/证据文件 → 类别（dependency_manifest | present） */
  ...
  /** 识别结果可能不完整（树被截断/取数预算耗尽/存在子模块） */
  truncated?: boolean;
  /** 扫描失败原因（no_repo_context / 上游错误摘要） */
  error?: string;
  scanned_at?: string;
  /** .gitmodules 声明的子模块识别结果（每项独立成功或带 error） */
  submodules?: DomainSubmoduleStack[];
}

/** 子模块仓库的紧凑技术栈投影（不含 languages/evidence，控 JSONB 体积） */
export interface DomainSubmoduleStack {
  /** 子模块在父仓库中的挂载路径 */
  path?: string;
  /** .gitmodules 段名 */
  name?: string;
  ...
  package_managers?: string[];
  containers?: string[];
  /** 子模块自身还声明了 .gitmodules（嵌套，未下钻）或取数受限 */
  truncated?: boolean;
  /** 该子模块扫描失败原因（unresolvable_url / 上游错误摘要） */
  error?: string;
  scanned_at?: string;
}
...
export interface DomainProjectIssue {
  ...
  requirement_document?: string;
  /** 问题状态（需求/bug 使用各自状态流） */
  status?: ConstsProjectIssueStatus | string;
  /** 问题摘要 */
  summary?: string;
  /** 问题标题 */
  title?: string;
  ...
  bug_reason?: string;
  /** 待确认项 */
  pending_items?: Array<{ id?: string; content?: string; status?: "pending" | "resolved" }>;
  /** 完成/修复说明 */
...
export interface DomainProjectAssociation {
  /** 关联行主键 */
  id?: string;
  /** 关联源项目 (A) */
  ...
  /** 关联类型: related | depends_on | ... */
  relation?: string;
  /** 节点工作区子目录名; null 时回退到 B 的 name */
  target_subdir?: string;
  /** 源项目 token 是否能读目标仓库 */
  can_read?: boolean;
  /** 源项目 token 是否能 push 目标仓库 */
  can_write?: boolean;
  created_at?: number;
  updated_at?: number;
  /** 目标项目摘要;can_read=false 时不带 repo_url */
  target_project?: {
    id?: string;
    name?: string;
    platform?: string;
...
export interface GithubComChaitinMonkeyCodeBackendDomainTeamOIDCPublicConfigResp {
  ...
  team_id?: string;
  team_name?: string;
  methods?: GithubComChaitinMonkeyCodeBackendDomainTeamOIDCPublicMethod[];
}

export interface GithubComChaitinMonkeyCodeBackendDomainTeamOIDCPublicMethod {
  method_id?: string;
  type?: string;
  display_name?: string;
  login_url?: string;
