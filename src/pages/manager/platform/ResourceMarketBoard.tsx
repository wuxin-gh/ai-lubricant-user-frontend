 ...
 * Tabs（全部 / MCP / Skill / 插件 / 项目提示词）、下方来源 / 技术栈 / 搜索筛选条，
 ...
 * - 市场 提示词（管理侧）：引用 / 取消引用
 * - 榜单 Skill/插件：安装（URL 导入），已安装置灰；榜单 提示词 只跳 GitHub
 * - 榜单 MCP：管理侧只跳 GitHub；用户侧保留原「个人远程 MCP 安装」入口
 */
...
import { useLeaderboardMarket } from "@/components/marketplace/LeaderboardMarketCards"
import {
  LANG_DOT,
  MODULE_BADGE_CLASS,
  SOURCE_BADGE_CLASS,
...
type Category = "all" | "mcp" | "skills" | "plugins" | "prompts"
...
export function ResourceMarketBoard({ userMode = false }: { userMode?: boolean }) {
  ...
  const load = useCallback(
    async (flush = false) => {
      ...
      try {
        ...
        const [names, refs, skills, plugins] = await Promise.all([
          userMode
  ...
  const installBoardItem = async (item: LeaderboardDiscoverItem) => {
    ...
    const kind: MirrorModule | null = mods.includes("skill") ? "skills" : mods.includes("plugin") ? "plugins" : null
    if (!kind) return
    ...
    try {
      if (kind === "skills") {
        ...
        const refFinal = spec.ref || ref
        const urlFinal = `https://github.com/${repo}/archive/refs/heads/${refFinal}.zip`
        if (entries.length === 0) {
          await importSkillUrl(url, { name: item.name || short, description: item.description || "", enabled: true })
        } else {
          for (const entry of entries) {
            const path = (entry.path || "").replace(/^\/+|\/+$/g, "")
            const skillMdPath = path ? `${path}/${entry.entry || "SKILL.md"}` : entry.entry || "SKILL.md"
            await importSkillUrl(urlFinal, {
  ...
  const rows = useMemo<Row[]>(() => {
    ...
    for (const module of modules) {
      ...
      if (sourceFilter !== "market") {
        for (const it of boardByModule[module] ?? []) {
          ...
          if (!matchQ([it.repo_full_name, it.description, it.language, ...(it.topics || []), ...(it.use_cases || [])])) {
            continue
          }
    ...
    return out
  }, [category, sourceFilter, stackTag, query, marketByModule, boardByModule])
  ...
  const renderBoardCard = (module: MarketModule, item: LeaderboardDiscoverItem) => {
    ...
    const downloadMcpBusy = busyKey === `board:${item.id}`
    return (
      <Card key={`board:${item.id}`} size="sm" className="shadow-none" style={busy ? { opacity: 0.6 } : undefined}>
        <CardContent className="flex h-full flex-col gap-2.5 px-4 py-3">
          <div className="flex items-start gap-2">
            <div className="min-w-0 flex-1">
              <a
              ...
              >
                <span className="truncate">{item.display_name || item.repo_full_name}</span>
          ...
          <div className="flex flex-wrap items-center gap-2 border-t pt-2">
            ...
            {isUserMcpInstall ? (
              <Button size="sm" disabled={downloadMcpBusy} onClick={() => void installBoardMcpUser(item)}>
                {downloadMcpBusy ? "安装中..." : "安装"}
              </Button>
            ) : null}
          </div>
        </CardContent>
