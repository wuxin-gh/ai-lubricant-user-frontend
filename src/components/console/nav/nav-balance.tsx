...
export default function NavBalance({
  ...
  // 无 C 端 session/role）均放行。仅在账号触发（头像栏）时展示于头像旁。
  const isEmergencyAdmin = typeof window !== "undefined" && !!localStorage.getItem("admin_access_token")
  ...
  const triggerContent = triggerMode === "account" ? (
    <div className="flex w-full min-w-0 items-center gap-2">
      <Avatar className="size-8 rounded-lg">
        <AvatarImage src={user?.avatar_url || publicUrl("/logo-light.png")} alt={user?.name || t("navBalance.common.unknownUser")} />
    ...
    <Dialog open={dialogOpen} onOpenChange={handleOpenChange}>
      {!hideTrigger && (
        variant === "header" ? (
          ...
          // 账号触发行：头像按钮 + 右侧「进入管理后台」箭头按钮（仅管理员可见）。
          // 箭头必须是触发按钮的兄弟节点，不能嵌在 DialogTrigger 内，否则点击箭头会误开账户弹框。
          <SidebarMenu>
            <SidebarMenuItem className="flex items-center gap-1">
              ...
              {triggerMode === "account" && showAdminEntry && (
                <Button
                  variant="ghost"
                  size="icon"
