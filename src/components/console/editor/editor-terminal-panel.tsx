...
import { deleteEditorTerminal, editorTerminalWs, listEditorTerminals, type EditorTerminal } from "@/api/editorClient"
...
export function EditorTerminalPanel({ editorId, onClosePanel }: EditorTerminalPanelProps) {
  const { resolvedTheme } = useTheme()
  const [sessions, setSessions] = useState<EditorTerminal[]>([])
  ...
  const fetchSessions = useCallback(async () => {
    try {
      const result = await listEditorTerminals(editorId)
      const next = [...(result.terminals || [])].sort((a, b) => (b.created_at || 0) - (a.created_at || 0))
      setSessions(next)
  ...
  async function closeSession(event: React.MouseEvent, terminalId: string) {
    ...
    try {
      ...
      await deleteEditorTerminal(editorId, terminalId)
  ...
  return (
    <div className="flex h-full min-h-0 overflow-hidden rounded-lg border">
              ...
              {displaySessions.map((session) => (
                <div key={session.id} className={cn("group flex items-center gap-2 rounded-md px-2 transition-colors", session.id === currentId ? "bg-muted text-foreground" : "text-muted-foreground hover:bg-muted/60")}>
                  <button type="button" className="flex min-w-0 flex-1 items-center gap-2 py-1 text-left" onClick={() => { setCurrentId(session.id); setSignal((value) => value + 1) }}>
                    ...
                    <span className="truncate text-xs">{titles[session.id] || session.title || session.id.slice(0, 8)}</span>
                  </button>
      ...
      <div className="min-h-0 flex-1">
        {currentId ? (
          <Terminal
            ws={editorTerminalWs(editorId, currentId)}
