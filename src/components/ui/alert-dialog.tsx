...
function AlertDialogOverlay({
  ...
  return (
    <AlertDialogPrimitive.Overlay
      ...
      className={cn(
        // z-[1100]：确认框（含遮罩）要盖过 z-[60] 的非模态浮窗（FloatingPanel），
        // 以及 Channels 等页面用内联 zIndex:1000 的自定义 Modal——嵌在其内的删除确认
        // 之前被 z-70 盖住。Toast 仍在 z-9999 之上。
        "fixed inset-0 z-[1100] bg-black/10 duration-100 supports-backdrop-filter:backdrop-blur-xs data-open:animate-in data-open:fade-in-0 data-closed:animate-out data-closed:fade-out-0",
        className
      )}
      {...props}
    />
