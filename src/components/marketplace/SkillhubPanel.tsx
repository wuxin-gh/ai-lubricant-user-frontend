/**
 * skillhub 技能源同步面板（市场管理 → 配置 → skillhub tab）。
 * SkillHub（skillhub.cn，腾讯云 dnspod 团队运营的国内 AI Skills 社区）的技能是公开
 * API，无需鉴权。配置与同步信息在同一张卡里，一个 tab 一个保存。
 */
...
import { syncSkillhub, fetchSkillhubLastSync, type SourceSyncStatus } from "@/api/marketplaceAdmin"
...
export function SkillhubPanel({ onSaved }: { onSaved?: () => void }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [src, lastSync] = await Promise.all([
        ...
        fetchSkillhubLastSync().catch(() => null),
      ])
      setConfig(src)
      setLast(lastSync)
      setEnabled(!!src.skillhub_enabled)
      const iv = src.skillhub_interval_hours || 24
  ...
  const handleSync = async (opts: { overwriteDraft: boolean; overwritePublished: boolean }) => {
    ...
    try {
      ...
      try { setLast(await fetchSkillhubLastSync()) } catch { /* 静默，hook 继续轮询 */ }
    } catch (err) {
  ...
  const save = async () => {
    ...
    try {
      await updateMarketplaceSourceConfig({
        skillhub_enabled: enabled,
        skillhub_interval_hours: interval,
      })
      toast.success("skillhub 配置已保存")
      await load()
      onSaved?.()
    } catch (err) {
  ...
  return (
    <div className="space-y-4">
      <Card className="shadow-none">
        <CardHeader className="pb-3"><CardTitle className="text-base">skillhub 技能源</CardTitle></CardHeader>
        <CardContent className="space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-2 rounded-md border bg-muted/20 px-3 py-2 text-sm">
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
              <span className="text-muted-foreground">社区 <span className="font-mono text-foreground">skillhub.cn</span></span>
              ...
              ) : config?.skillhub_last_sync_at ? (
                // 重启后内存没了：回落配置里持久化的上次同步时间
                <span className="text-muted-foreground">
                  上次 {fmtSyncAt(config.skillhub_last_sync_at)}（历史记录）
                </span>
          ...
          <Separator />
          <Alert><AlertDescription>SkillHub（skillhub.cn）是国内 AI Skills 社区，技能是公开 API，无需鉴权。同步时按 score 排序分页拉列表，逐个下载 zip，转 skill manifest 落库（category→分类，subCategories.name→标签）。</AlertDescription></Alert>
